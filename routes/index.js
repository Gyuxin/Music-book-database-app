...
exports.api_songs = function(request, response) {
  ...
  if (urlObj.query['title']) {
    ...
    sql = "SELECT id, title FROM songs WHERE title LIKE '"+title+"' OR composer LIKE '"+title+"' ORDER BY title LIMIT 30";
  ...
  if (urlObj.query['bookcode']) {
    search_content = `%${urlObj.query['bookcode']}%`
    sql = "SELECT rowid, bookcode, title FROM bookcodes WHERE bookcode LIKE '"+search_content+"' OR title LIKE '"+search_content+"' ORDER BY bookcode";
    console.log("finding bookcode: "+urlObj.query['bookcode'])
    //data object to send to client
    db.all(sql,function(err, rows) {
      for (let i = 0; i < rows.length; i++) {
        let song = {}
        song.bookcode = rows[i].bookcode
        song.title = rows[i].title
