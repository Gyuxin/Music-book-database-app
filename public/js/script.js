function bookUpdateHandler() {
  //============================
  let book_bookcode = document.getElementById('book_book_field').value
  ...
  let book_page_offset = document.getElementById('book_pageoffset_field').value
  let book_num_pages = document.getElementById('book_numpage_field').value
  ...
  xhr.send(JSON.stringify(bookdata)) //send JSON data to server
}
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      */
      ...
      songDetailsDiv.innerHTML += `<div class="input_lable">BOOKCODE: <input class="details" type="text" id="book_book_field" value="${response.bookcode}"/></div>`
      ...
      songDetailsDiv.innerHTML += `</div>`
      songDetailsDiv.innerHTML += `<div class="details_wrapper">`
      songDetailsDiv.innerHTML += `<div class="input_lable">PAGE OFFSET: <input class="details" type="text" id="book_pageoffset_field" value="${response.page_offset}"/></div>`
      songDetailsDiv.innerHTML += `</div>`
      songDetailsDiv.innerHTML += `<div class="details_wrapper">`
      songDetailsDiv.innerHTML += `<div class="input_lable">NUM PAGE: <input class="details" type="text" id="book_numpage_field" value="${response.num_pages}"/></div>`
      songDetailsDiv.innerHTML += `</div>`
      songDetailsDiv.innerHTML += `<div></div>`
      songDetailsDiv.innerHTML += `<button id="song_update" onclick="bookUpdateHandler()" >Update</button>`
    }
  }
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      */
      ...
      songDetailsDiv.innerHTML += `</div>`
      songDetailsDiv.innerHTML += `<div class="details_wrapper">`
      songDetailsDiv.innerHTML += `<div class="input_lable">STUDENTNUM: <input class="details" type="text" id="song_studentnum_field" value="${response.studentnum}"/></div>`
      songDetailsDiv.innerHTML += `</div>`
      songDetailsDiv.innerHTML += `<div></div>`
...
function titleSearchBookHandler() {
  //===============================
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      for (let song of response.songs) {
        songDiv.innerHTML = songDiv.innerHTML + `<li onclick="bookSelectHandler('${song.bookcode}')">
<font color="blue">${song.bookcode}:  ${song.title}</font></li>`
      }
      songDiv.innerHTML = songDiv.innerHTML + `</ul>`
    }
  }
